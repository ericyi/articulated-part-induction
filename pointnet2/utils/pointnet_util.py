...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, bn=True, pooling='max', tnet_spec=None, knn=False, use_xyz=True, mask=None, centralize_points=False):
    ...
    with tf.variable_scope(scope) as sc:
        if group_all:
            ...
            new_xyz, new_points, idx, grouped_xyz = sample_and_group(npoint, radius, nsample, xyz, points, tnet_spec, knn, use_xyz, centralize_points)
        ...
        elif pooling=='maskmax':
            ## only used for groupall
            # mask (batch_size, npoint)
            # new_points (batch_size, 1, npoint, mlp[-1])
            mask = tf.expand_dims(tf.expand_dims(mask,1),-1)
            new_points = tf.reduce_max(tf.multiply(new_points,mask), axis=[2], keep_dims=True)
        elif pooling=='min':
...
def pointnet_sa_module_msg(xyz, points, npoint, radius_list, nsample_list, mlp_list, is_training, bn_decay, scope, bn=True, use_xyz=True, centralize_points=False, new_xyz=None, central_points=None):
    ...
    with tf.variable_scope(scope) as sc:
        fpsidx = farthest_point_sample(npoint, xyz)
        if new_xyz is None:
            new_xyz = gather_point(xyz, fpsidx)
        new_points_list = []
        ...
        return new_xyz, new_points_concat, fpsidx
