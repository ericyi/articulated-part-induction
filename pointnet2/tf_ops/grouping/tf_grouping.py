...
if __name__=='__main__':
    ...
    import time
    np.random.seed(101)
    pts = np.random.random((1,10,64)).astype('float32')
    tmp1 = np.random.random((1,10,3)).astype('float32')
    ...
    with tf.Session('') as sess:
        ...
        print ret
        '''
        xyz1_eval, grouped_xyz_eval = sess.run([xyz1, grouped_xyz])
        print(xyz1_eval.shape, grouped_xyz_eval.shape)
        print(xyz1_eval, grouped_xyz_eval)
